...
- 丢弃任何视觉都必须给出受控理由，不允许静默消失：`repeated-chrome`（校徽、水印、模板装饰、页脚条）、`decorative`（无信息量的剪贴画/图库照片）、`redundant-with-text`（示意图的内容正文已完整陈述）、`duplicate`、`illegible`、`superseded-by-table`。
- **反复出现的模板外壳由脚本自动识别**：位置与尺寸在超过一半页面上重复的视觉块判定为 `repeated-chrome`，草稿台账里直接标好。校徽不需要被逐页判断 44 次。
- 拿不准就保留。多留一张图读者只多看一眼；漏掉一张图，读者丢失的是一个概念，而且笔记里看不出它曾经存在。
...
- Canvas 卡片余量是双向约束：扣除本机渲染器的34px内部占位后，每张文字卡必须保留8–12px；太小会裁切，太大说明卡片过松，也会失败。
- Canvas 顶部紫色摘要与红色原文预览构成动态高度区域；下方模块按其最终底边再下移80px，DOM重排后禁止继续使用固定`y=520`。
