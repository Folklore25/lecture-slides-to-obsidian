...
# The apply script writes through the filesystem. A CLI path here would reintroduce the
# cold-start hazard into the live-lecture workflow, where the app is often not running.
if grep -qE '"obsidian", "(read|create|eval|open|version)"' "$live_notes_skill_dir/scripts/apply-note-patches.py"; then
	printf 'the live-notes apply script must not call the Obsidian CLI\n' >&2
	exit 1
fi
