Use `$obsidian-canvas-designer` to complete a Canvas-only task for **one** note.
...
Do not invoke extraction, MinerU, credentials, course routing, note writing, or conversion reports. Do not edit the complete note. When a document produced several notes, each note gets its own task and its own staging paths; never combine two notes in one subagent.
...
If exact H2 anchors are missing, return their H3 candidates and stop without modifying the note. Omit `source_page` when the note carries no page markers; it is optional for content-driven notes.
