...
- Explicitly invoke `obsidian-markdown`, `obsidian-cli`, and `obsidian-canvas-designer`; availability alone is not loading. Pass all three names to `preflight.py --loaded-skill` and record them in temporary QA context. Also invoke and pass `slide-layout-refiner`, which is enabled by default.
...
- Slide-layout refinement is enabled by default. First write the base note to its final path, then delegate the original PDF plus that note to `slide-layout-refiner` with a model that supports visual input. It directly overwrites the same note, preserves every source-page boundary, and rolls back from an outside-vault snapshot if validation fails. Pass `--no-visual-layout-refinement` to skip it entirely, or `--layout-visual-input false` to skip it when the selected model cannot view the PDF.
...
8. Write the base Markdown to its final path. If optional LaTeX normalization is enabled, load `obsidian-latex-refiner` and run `normalize-latex.py` against that note with an outside-vault snapshot and report; it rewrites math syntax in place and restores the snapshot on any conservation failure. Then, unless slide-layout refinement is disabled, snapshot the note outside the vault, load `slide-layout-refiner`, and delegate the source PDF plus that target path to a multimodal subagent. The subagent directly overwrites the target. Validate the overwrite; on failure the validator restores the snapshot automatically. Never cross or alter source-page markers.
...
- Load [../slide-layout-refiner/SKILL.md](../slide-layout-refiner/SKILL.md) by default; it is enabled unless `--no-visual-layout-refinement` is passed or the model cannot view the PDF.
