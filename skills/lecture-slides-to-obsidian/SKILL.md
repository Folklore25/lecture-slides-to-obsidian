...
- The Obsidian CLI is called from exactly one place: the Canvas DOM measurement inside `obsidian-canvas-designer/scripts/canvas-render-qa.py`. Vault reads and writes go through the filesystem, which is simpler.
- **Never run the Obsidian CLI while Obsidian is not running.** This is the one hard rule. With the app up, CLI calls talk to it and do not steal focus, so ordinary use is fine. With the app down, the CLI cold-starts Obsidian: the window pops to the front, the app checks for updates, and the command **never returns** — measured, three commands produced three launches and three window pop-outs. Confirm the app is running before any CLI call, bound every call with a timeout, and never let a CLI call be the thing that launches Obsidian.
