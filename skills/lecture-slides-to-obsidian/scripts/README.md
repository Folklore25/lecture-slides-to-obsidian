...
- `plan-conversion-batch.py` — plans one-subagent-per-file conversion dispatch for two or more source files. Validates per-file isolation (unique id, source, document folder, staging; source outside the vault, document folder inside it, staging outside it), emits parallelism and waves, and keeps shared state (routing, registry) with the main Agent. It never plans a Canvas.
- `plan-canvas-batch.py` — validates per-Canvas isolation and describes the lease-guarded Canvas lane: authoring parallelises freely, and the DOM step is serialized by an exclusive cross-process lease.
- The lease itself lives in the sibling `obsidian-canvas-designer/scripts/obsidian-gui-lock.py`, and `canvas-render-qa.py` takes it automatically around the DOM step, so concurrent agents queue instead of conflicting or being forbidden to run.
