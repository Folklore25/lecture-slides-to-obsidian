...
- `plan-canvas-batch.py` validates isolated one-document Canvas tasks, requires subagent delegation for batches of two or more, and reserves a single renderer lane.
...
- `validate-output.py <document-folder> --vault-root <vault-root> --report <staging-report> --recall-model <staging-model> [--layout-refinement-report <layout-report>] [--latex-refinement-report <latex-report>] --aesthetic-check <aesthetic> --render-metrics <metrics> --render-check <check>` validates final artifacts; `--delete-qa-on-success` removes all temporary QA files.
