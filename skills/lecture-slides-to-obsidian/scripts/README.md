...
- `plan-canvas-batch.py` — validates per-Canvas isolation and emits the exclusive serial Canvas lane (`parallelism: 1`, owner `main-agent`, exclusive resource `obsidian-app-gui`). Canvas is never fanned out.
