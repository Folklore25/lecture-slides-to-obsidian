...
The apply tool has a single filesystem backend, `--backend fs`: it reads and writes the vault file directly with an atomic replace plus SHA readback, and never calls the Obsidian CLI. The former `obsidian-cli` backend was removed because every `obsidian` invocation activates the Obsidian window - unacceptable during a live lecture, and unnecessary for a write Obsidian picks up from disk anyway.
